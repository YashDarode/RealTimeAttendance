        ...
        if faceCurFrame:
            for encodeFace, faceLoc in zip(encodeCurFrame, faceCurFrame):
                ...
                if matches[matchIndex] and faceDis[matchIndex] < 0.5:
                    ...
                    cv2.putText(imgBackground, "Face not recognized", (275, 400), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2)
                    modeType = 0
                    counter = 0
            ...
            if counter != 0 and id != -1:
                ...
                if studentInfo:
                    ...
                    datetimeObject = datetime.strptime(last_attendance_time, "%Y-%m-%d %H:%M:%S") if last_attendance_time else None
                    secondsElapsed = (datetime.now() - datetimeObject).total_seconds() if datetimeObject else float('inf')

                    # Ensure that attendance is only marked using live feed
...
def add_student():
    if request.method == 'POST':
        ...
        img_path = os.path.join('D:\RealTimeAttendance\Images', file_name)
