    ...
    if faceCurFrame:
        for encodeFace, faceLoc in zip(encodeCurFrame, faceCurFrame):
            ...
            if matches[matchIndex] and faceDis[matchIndex] < 0.5:
                ...
                # No match found - this face does not exist in the system
                cv2.putText(imgBackground, "Face not recognized", (275, 400),
                            cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2)
                cv2.imshow("Face Attendance", imgBackground)
                cv2.waitKey(1)
                # Reset the mode or keep scanning for other faces
                modeType = 0
                counter = 0
